...
def run():
    ...
    # Merge SoSoValue intelligence if available
    soso_path = ROOT_DIR / "data" / "raw" / "sosovalue_latest.json"
    if soso_path.exists():
        try:
            soso_data = json.loads(soso_path.read_text())
            modules = soso_data.get("modules", {})
            out["sosovalue"] = {
                "news_sentiment": modules.get("news", {}).get("sentiment", {}),
                "macro_impact": modules.get("macro", {}),
                "etf_signal": modules.get("etf", {}),
                "treasury_signal": modules.get("treasuries", {}),
                "indices": modules.get("indices", []),
                "market_overview": modules.get("market", {}).get("assets", []),
                "sectors": modules.get("sectors", {}),
                "fetched_at": soso_data.get("timestamp"),
            }
            print(f"[OK] SoSoValue intelligence merged")
        except Exception as e:
            print(f"[WARN] Erreur lecture sosovalue_latest.json: {e}")
